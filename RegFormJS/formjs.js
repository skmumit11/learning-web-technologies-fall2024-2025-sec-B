...
const fieldTypes = {
    ...
    email: 'email',
    dob: 'date',
    gender: 'radio',  // Use radio buttons for gender
    bloodGroup: 'select' // Use select dropdown for blood group
};
...
document.getElementById('addFieldBtn').addEventListener('click', function() {
    ...
    if (fieldName && !fieldsToAdd.includes(fieldName)) {
        fieldsToAdd.push(fieldName);
        renderDynamicForm();
        document.getElementById('fieldSelect').value = ''; // Clear the dropdown after adding
    } else {
        alert('Please select a valid field name (or the field already exists).');
    }
});
